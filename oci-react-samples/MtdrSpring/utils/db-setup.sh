...
CREATE TABLE TODOUSER.departamento (id NUMBER GENERATED ALWAYS AS IDENTITY, nombre VARCHAR2(4000) NOT NULL, PRIMARY KEY (id));
CREATE TABLE TODOUSER.tarea (id NUMBER GENERATED ALWAYS AS IDENTITY, nombre VARCHAR2(4000) NOT NULL, descripcion VARCHAR2(4000) NOT NULL, fechaCreacion TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, estatus NUMBER(1,0), fechaLimite TIMESTAMP WITH TIME ZONE NOT NULL, tipoTarea VARCHAR2(4000) NOT NULL, PRIMARY KEY (id), FOREIGN KEY(id) references tarea(id));
CREATE TABLE TODOUSER.proyecto (id NUMBER GENERATED ALWAYS AS IDENTITY, nombre VARCHAR2(4000) NOT NULL, fechaInicio TIMESTAMP WITH TIME ZONE, fechaFin TIMESTAMP WITH TIME ZONE, estatus NUMBER(1,0), PRIMARY KEY (id), FOREIGN KEY(id) references departamento(id));
CREATE TABLE TODOUSER.empleado (id NUMBER GENERATED ALWAYS AS IDENTITY, nombre VARCHAR2(4000) NOT NULL, apellido VARCHAR2(4000) NOT NULL, correo VARCHAR2(4000) NOT NULL, celular VARCHAR2(4000) NOT NULL, direccion VARCHAR2(4000) NOT NULL, estatus NUMBER(1,0), manager NUMBER(1,0), PRIMARY KEY (id), FOREIGN KEY(id) references departamento(id), FOREIGN KEY(id) references empleado(id));
commit;
!
